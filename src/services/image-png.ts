...
export type ConvertDocImagesToPngReport = {
...
const EMPTY_REPORT: ConvertDocImagesToPngReport = {
  scannedImageCount: 0,
...
export async function convertDocImagesToPng(docId: string): Promise<ConvertDocImagesToPngReport> {
  ...
  for (const assetPath of allImagePaths) {
    if (!isConvertibleImageAssetPath(assetPath, "png")) {
      skippedImageCount += 1;
      continue;
    }
    try {
      const result = await convertLocalAssetImageToPng(assetPath);
      if (!result.converted) {
        skippedImageCount += 1;
        continue;
      }
